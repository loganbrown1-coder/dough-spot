...
4. Before deploying, add five environment variables from your `.env.local` (**Project Settings > Environment Variables** in Vercel): `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `SITE_URL` (set this one to your `*.vercel.app` URL, or your custom domain), and `CRON_SECRET` (generate one with `openssl rand -hex 32` - see [Photo retention](#photo-retention) below for what it's for).
